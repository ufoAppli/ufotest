...
import type { Entry, Masters, Photo, Session, Store } from "../types"; // ★ import type に変更
...
export type AppState = {
  ...
  init: (masters: Masters) => Promise<void>;
  createStore: (name: string) => Promise<string>;
  deleteCurrentStore: () => Promise<void>;
  setSession: (partial: Partial<Session>) => void;
  clearPrevStepData: (currentStep: number) => void;
  upsertEntryWithPhotos: () => Promise<void>;
  refreshStores: () => Promise<void>;
  getEntriesForCurrentStore: () => Promise<Entry[]>;
};
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  init: async (masters: Masters): Promise<void> => {
    set({ masters });
  ...
  createStore: async (name) => {
    ...
    const store: Store = { id, name, createdAt: now };
  ...
  clearPrevStepData: (currentStep: number) => {
    const s = { ...get().session };
  ...
  upsertEntryWithPhotos: async () => {
    ...
    if (!currentStoreId || !session.machine || !session.prize || !session.size || !session.setup) return;
